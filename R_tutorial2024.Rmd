...
As we known, there is a variable type called logic, `TRUE` and `FALSE`.

* x==y: Is $x=y$?
* x!=y: Is $x\neq y$?
* x>=y: Is $x\geq y$? (x<=y: Is $x\leq y$?)
* x, y can be integer and character.
...
* check the character in a string
* use the function `grepl`
* ex: Is "a" in the string "apple"?
...
* `if(condition){do}`
* `do`: if the condition is true, then do!
...
* use `for(i in vector)`
* `i`: the variable in the loop
* `vector`: the collection of variables for each loop
* ex: I want build a "101" vector.
...
* ex: evaluate $1\times2\times3\times\cdots\times100$.

```{r class.output="code-output"}
Multiple_100 <- 1

for(i in 1:100){
  Multiple_100 <- Multiple_100*i
}
Multiple_100
```

* ex: print the name list

```{r class.output="code-output"}
Name_list <- c("Sam", "Lan", "Elaine", "Yuji", "Yuki")
