...
## 0 前言
   ...
   在正式進入正題之前，我們先來看個例子，這是一個在分析上很簡單也很經典的例子，資料顯示的是瑞士的銀行他們好奇說有沒有辦法使用鈔票的各項數據去分辨真假鈔，他們抽樣了各100張的真假鈔，共200張來進行數據測量，量每張鈔票的長度、上下左右的寬度和對角線，共6組數據。
...
![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/0-4.jpg)
   ...
   好奇之下，我們畫了這張散布圖，並用鈔票的真假去上色，果不其然跟我們猜想的一樣，單單靠這兩項數據既可以分別真假鈔了，這就是最簡單、初步使用R去看資料的方式。

## 1 R

   R是語言，是我們跟電腦溝通的語言，電腦也必須有R語言，雖然R原本就內建跑程式的Compiler，但是因為是原始的工具，所以介面、功能等都非常陽春，所以我們建議使用R Studio，除了介面簡單易懂，功能完整且強大。
...
![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/1-4.jpg)
   ...
   任何的文字放進雙引號(或單引號)裡就會成為字元或字串，但是要注意的是，比如說，我想要打Stuednt's那我就必須注意單引號或雙引號，只能打"Stuednt's"，才會成功區隔開來。

### 1.1.4 vector

![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/1-5.jpg)

   與函數一樣使用，`c()`把我們想組成向量的值放進c這個函數哩，就可以跑出向量。

### 1.1.5 factor

![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/1-6.jpg)

   由向量衍伸而來的，只是改變變數的型態而已，主要是作為分類使用，也方便看出種類。

### 1.1.6 matrix

![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/1-7.jpg)

   矩陣`matrix()`裡，放入了一個向量，將這個向量重新排列成矩陣的形式。圖中分行的寫法更直覺地看出矩陣實際的長相，要注意的是除了`ncol`和`nrow`的乘積要是向量的個數外，後面還有一個`byrow`，如果填入`TRUE`照「列」(row)的順序填入向量的值，反之，則是「行」(column)。

### 函數 (function)和參數 (parameter)的設定

   眼尖的你們會發現`<-`和`=`的使用時機，在R語言中做的區隔，所有任何`function()`這種形式的變數，在R裡都是函數，無論是向量的`c()`、`factor()`或是`matrix()`都是函數，函數裡有許多必須填寫的設定，我們稱為參數(parameter)並用`=`做設定，只有設定變數才會用`<-`。

   舉例來說，矩陣的參數除了向量之外，`ncol`、`nrow`和`byrow`等都是函數`matrix()`的參數，使用`=`。如果使用`<-`是不行的，會運行失敗。

   除此之外，許多參數其實是有默認的設定的，像矩陣中的`byrow = FALSE`就是默認的。舉函數來說，我寫`f <- function(x, y = 2){x*y}`，這時我如果跑`f(3,2)`、`f(x=3,y=2)`或`f(x=3)`都會得到一樣的結果，`y=2`是默認的參數設定。順帶一提，有些參數是有限制的，可能必須是數值、必須是向量等。

   總而言之，函數裡的參數設定一率是用`=`，變數的設定則是都可以。
...
### 4.1 Students' t-Test
   ...
   假設檢定 (Hypothesis Test)是統計學裡，非常重要的一個篇章，不過這邊只教在R上怎麼作使用，有興趣可以看[課本](https://cis.temple.edu/~latecki/Courses/CIS2033-Spring13/Modern_intro_probability_statistics_Dekking05.pdf)的第25章。想法就是假設一個預期的情況，在用統計量去檢定這個假設是否成立。

![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/6-1.jpg)

   舉例而言，在上個章節中有一個100人的資料，我好奇平均身高是不是170公分。檢定當中又分為雙尾和單尾，這裡所使用的雙尾檢定，差別就在於 $H_1$ 對立假設的條件，下方的投影片就是屬於單尾。

![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/6-2.jpg)

再來也可以檢定兩群資料的差別，下方也是使用上個章節所創造出的資料，我們好奇的事情是男生 ( $\mu_1$ )的平均BMI是不是低於女生 ( $\mu_2$ )的，以正常標準來說，女生的正常值是比男生來的高的，但檢定出來的結果是相反的。

![](https://raw.githubusercontent.com/YiHsinLu/2024R.io/main/fig/ppt/6-3.jpg)

假設檢定除了是統計上常用的工具外，在很多統計資料或隨機分配的情況下都可以用來測試我們想像中的事情是否成立。在日常生活中有太多事情是不可預期也無法精確估計的，如果真的有興趣知道這些隨機的現象，那統計學對我們來說就非常重要。

## 結語

感謝能耐心看到這裡的各位，希望我寫的這些內容可以幫助大家更了解R和統計，如果有任何問題或覺得我說的內容有誤也歡迎與我聯繫，謝謝！
...
* 類別型變數(categorical)：像是性別這類可以將人分類的
* 時間型變數：日期、時間等

另外就是最特殊、處理方式也最多種的多值變數(multi-value)，也是我們一般所說的多選。
